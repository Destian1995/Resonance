// ============ INPUT — touch joystick + keyboard ============
const Input = {
    mobile: false,
    dx: 0, dy: 0,         // movement vector
    tapped: false,         // single tap this frame

    // Joystick state
    joy: { active: false, id: -1, bx: 0, by: 0, sx: 0, sy: 0 },
    // Any other touch (for menu taps)
    tapX: 0, tapY: 0,
    ...
    _sc(e) {
        ...
        this.tapX = (e.clientX - r.left) * (this.c.width / r.width);
        this.tapY = (e.clientY - r.top) * (this.c.height / r.height);
    },
    ...
    _ts(e) {
        ...
        for (const t of e.changedTouches) {
            const p = this._pos(t);
            // Left half → joystick
            if (p.x < this.c.width * 0.5 && !this.joy.active) {
                this.joy.active = true;
                this.joy.id = t.identifier;
                this.joy.bx = p.x; this.joy.by = p.y;
                this.joy.sx = p.x; this.joy.sy = p.y;
            } else {
                this.tapped = true;
                this.tapX = p.x; this.tapY = p.y;
            }
        }
    },
    ...
    _tm(e) {
        e.preventDefault();
        for (const t of e.changedTouches) {
            if (t.identifier === this.joy.id && this.joy.active) {
                const p = this._pos(t);
                const dx = p.x - this.joy.bx, dy = p.y - this.joy.by;
                const d = Math.sqrt(dx*dx + dy*dy);
                const max = CFG.JOY_RADIUS;
                if (d > CFG.JOY_DEAD) {
                    const cl = Math.min(d, max);
                    this.dx = (dx/d) * (cl/max);
                    this.dy = (dy/d) * (cl/max);
                    this.joy.sx = this.joy.bx + (dx/d) * cl;
                    this.joy.sy = this.joy.by + (dy/d) * cl;
                } else {
                    this.dx = 0; this.dy = 0;
                    this.joy.sx = this.joy.bx; this.joy.sy = this.joy.by;
                }
            }
        }
    },
    ...
    drawJoy(ctx) {
        ...
        if (this.joy.active) {
            ctx.globalAlpha = 0.2;
            ctx.fillStyle = '#fff';
            ctx.beginPath(); ctx.arc(this.joy.bx, this.joy.by, CFG.JOY_RADIUS, 0, Math.PI*2); ctx.fill();
            ctx.globalAlpha = 0.5;
            ctx.fillStyle = '#0ff';
            ctx.beginPath(); ctx.arc(this.joy.sx, this.joy.sy, 18, 0, Math.PI*2); ctx.fill();
            ctx.globalAlpha = 1;
        }
    }
};
